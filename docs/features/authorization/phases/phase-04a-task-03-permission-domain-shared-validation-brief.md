...
- **Author:** Verin
- **Scope:** Task 3 only. Bootstrap `Aiel.Permissions.Domain.Shared` and `Aiel.Permissions.Domain.Shared.UnitTests` without mixing Task 4 grant behavior, permission runtime, adapters, or generator/analyzer feature work.
...
- **Baseline observed in this worktree:**
  - `Aiel.Permissions.Domain.Shared` does not exist yet.
  - `Aiel.Permissions.Domain.Shared.UnitTests` does not exist yet.
...
Treat `Aiel/tests/Aiel.Permissions.Domain.Shared.UnitTests/Aiel.Permissions.Domain.Shared.UnitTests.csproj` as the authoritative acceptance gate for Task 3.

Task 3 is not done until all of the following are true:
1. `Aiel/src/Aiel.Permissions.Domain.Shared/Aiel.Permissions.Domain.Shared.csproj` and `Aiel/tests/Aiel.Permissions.Domain.Shared.UnitTests/Aiel.Permissions.Domain.Shared.UnitTests.csproj` exist and are added to `TwoRivers.slnx`, `Aiel.slnx`, and `virtual-folders.json`.
2. `dotnet test .\Aiel\tests\Aiel.Permissions.Domain.Shared.UnitTests\Aiel.Permissions.Domain.Shared.UnitTests.csproj --nologo --verbosity minimal` passes cleanly.
...
1. **`PermissionDomainSharedSurfaceTests`**
   ...
   - Fail if `PermissionLifecycle` or `PermissionGrantDecision` is missing from the shared package.
