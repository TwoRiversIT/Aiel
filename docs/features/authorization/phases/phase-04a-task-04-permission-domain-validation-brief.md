...
- **Author:** Verin
- **Scope:** Task 4 only. Create `Aiel.Authorization.Domain` and `Aiel.Authorization.Domain.UnitTests` for the first permission domain model slice. Do not mix Task 5+ contracts, runtime services, EF mappings, analyzers, generators, or ASP.NET adapters into this change.
...
- **Baseline observed in this worktree:**
  - `Aiel/src/Aiel.Authorization.Domain/` does not exist yet.
  - `Aiel/tests/Aiel.Authorization.Domain.UnitTests/` does not exist yet.
  - `Aiel.Authorization.Domain.Shared.UnitTests` is currently green: 7/7 passing.
...
Treat `Aiel/tests/Aiel.Authorization.Domain.UnitTests/Aiel.Authorization.Domain.UnitTests.csproj` as the authoritative acceptance gate for Task 4.
...
5. Grant-creation tests prove the aggregate fails closed for invalid construction inputs:
   ...
   - missing explicit `AuthorizationGrantDecision`
...
1. **`PermissionGrantCreationTests`**
   ...
   - Fail until the aggregate requires an explicit `AuthorizationGrantDecision` instead of assuming a default branch.
...
4. **`PermissionDomainSurfaceTests`**
   - Fail until the domain assembly exposes the grant aggregate plus one catalog root in `Aiel.Authorization.Domain`.
   - Fail if public properties return nullable values, raw strings, or raw IDs where Task 3 value objects already exist.
   - Fail if Task 4 types leak into `Aiel.Authorization.Domain.Shared` or depend outward on application/infrastructure packages.
