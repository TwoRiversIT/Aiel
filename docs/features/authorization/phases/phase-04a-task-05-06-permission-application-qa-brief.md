...
- **Author:** Verin
- **Scope:** Tasks 5 and 6, treated as one atomic slice. Create `Aiel.Permissions.Application.Contracts`, `Aiel.Permissions.Application`, and `Aiel.Permissions.Application.UnitTests`. Do not mix EF mappings, ASP.NET adapters, analyzers, generators, or client-side capability concerns into this change.
...
- **Baseline observed in this worktree:**
  - `Aiel/src/Aiel.Permissions.Application.Contracts/` does not exist yet.
  - `Aiel/src/Aiel.Permissions.Application/` does not exist yet.
  - `Aiel/tests/Aiel.Permissions.Application.UnitTests/` does not exist yet.
  - `Aiel.Permissions.Domain.UnitTests` is green: all tests passing.
  - `Aiel.Permissions.Domain.Shared.UnitTests` is green: all tests passing.
...
Treat `Aiel/tests/Aiel.Permissions.Application.UnitTests/Aiel.Permissions.Application.UnitTests.csproj` as the authoritative acceptance gate for Tasks 5+6.
...
2. `dotnet test .\Aiel\tests\Aiel.Permissions.Application.UnitTests\Aiel.Permissions.Application.UnitTests.csproj --nologo --verbosity minimal` passes cleanly.
3. `dotnet test .\Aiel\tests\Aiel.Permissions.Domain.UnitTests\Aiel.Permissions.Domain.UnitTests.csproj --nologo --verbosity minimal` still passes. Tasks 5+6 must not weaken the Task 4 domain contract.
4. `dotnet test .\Aiel\tests\Aiel.Permissions.Domain.Shared.UnitTests\Aiel.Permissions.Domain.Shared.UnitTests.csproj --nologo --verbosity minimal` still passes.
5. The contracts project depends only on `Aiel.Application.Contracts`, `Aiel.Permissions.Domain`, and `Aiel.Results`. It must not carry references to EF, HTTP, or any other infrastructure assembly.
6. The implementation project depends only on `Aiel.Permissions.Application.Contracts` and `Aiel.Permissions.Domain`. It must not reference EF, HTTP, or any concrete infrastructure assembly.
7. The test project depends on `Aiel.Permissions.Application.Contracts`, `Aiel.Permissions.Application`, and xUnit. All test doubles (fakes for `IPermissionStore`, `IActionValidator<TAction>`, `IActionPermissionChecker<TAction>`) are hand-written inline in the test project; no mocking framework is required.
...
All of the following must exist in `Aiel.Permissions.Application.Contracts` in the `Aiel.Permissions` namespace:
...
`IPermissionStore` must accept and return domain value objects (`PermissionName`, `PermissionSubjectTypeName`, `PermissionSubjectKey`, `PermissionScopeTypeName`, `PermissionScopeKey`, `PermissionGrantDecision`) and strong IDs. It must not expose EF entity types, nullable columns, or raw persistence primitives at the contract boundary.

## Required implementations (Task 6 scope)

All of the following must exist in `Aiel.Permissions.Application` in the `Aiel.Permissions` namespace:
...
- Add `Aiel.Permissions.Application.Contracts` with all interfaces and `PermissionErrors`.
- Add `Aiel.Permissions.Application` with `DefaultActionGate<TAction>` and `DefaultPermissionManager` that throw `NotImplementedException`.
- Add `Aiel.Permissions.Application.UnitTests` with all four test classes below.
...
4. **`PermissionApplicationSurfaceTests`**
   - Compile-fails until the contracts assembly contains `IActionGate<TAction>`, `IActionValidator<TAction>`, `IActionPermissionChecker<TAction>`, `IPermissionStore`, `IPermissionManager`, and `PermissionErrors` in the `Aiel.Permissions` namespace.
   - Fails if `PermissionErrors` members are exception types instead of `Error` values.
   - Fails if any public interface member returns or accepts a nullable reference type where a value object or strong ID already exists in `Aiel.Permissions.Domain.Shared`.
...
- Task 5 contracts reference `Aiel.Permissions.Domain` internal types (aggregates, domain events) at the contract boundary instead of `Aiel.Permissions.Domain.Shared` value objects and strong IDs.
