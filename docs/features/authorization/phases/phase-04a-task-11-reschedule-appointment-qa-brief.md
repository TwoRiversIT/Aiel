...
- **Baseline:** Task 10 fixes are in place. `PermissionDefinitionManifest` now carries action type, lifecycle, and previous names, the generator emits matching manifest data, rename fixtures live in `Aiel.Authorization.Testing`, and the current permission baseline is green in `Aiel.Authorization.Application.UnitTests`, `Aiel.Authorization.Testing.UnitTests`, `Aiel.Authorization.Generators.UnitTests`, and `Aiel.Authorization.EntityFrameworkCore.IntegrationTests`.
...
1. The phase plan explicitly keeps the Task 10 rename proof independent of the production slice. The rename test uses fixture actions from `Aiel.Authorization.Testing`, not the Task 11 command.
2. The Task 10 contract mismatch is closed in `Aiel/src/Aiel.Authorization.Application.Contracts/Aiel/Authorization/PermissionDefinitionManifest.cs` and `Aiel/src/Aiel.Authorization.Generators/Aiel/Authorization/Generators/PermissionDefinitionSourceGenerator.cs`.
3. The rename fixture seam exists already in `Aiel/src/Aiel.Authorization.Testing/Aiel/Authorization/Testing/Fixtures/PermissionFixtureActions.cs` and `Aiel/src/Aiel.Authorization.Testing/Aiel/Authorization/Testing/PermissionTestData.cs`.
4. The rename and manifest snapshot behavior is already guarded by `Aiel/tests/Aiel.Authorization.EntityFrameworkCore.IntegrationTests/Aiel/Authorization/PermissionMigrationTests.cs`.
...
1. Generator regression:
   - `Aiel/tests/Aiel.Authorization.Generators.UnitTests/Aiel/Authorization/Generators/PermissionDefinitionSourceGeneratorTests.cs`
   - Keep stable ID and manifest emission coverage green for `RescheduleAppointment`.

2. Persistence / rename regression:
   - `Aiel/tests/Aiel.Authorization.EntityFrameworkCore.IntegrationTests/Aiel/Authorization/PermissionMigrationTests.cs`
...
- Put gate-order and application-service orchestration tests in `Aiel.Authorization.Application.UnitTests`.
- Put fake sample IDs, fake repository helpers, and reference-slice support seams in `Aiel.Authorization.Testing` and `Aiel.Authorization.Testing.UnitTests`.
