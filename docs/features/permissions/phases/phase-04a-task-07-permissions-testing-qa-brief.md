# Phase 04a Task 7 — Aiel.Permissions.Testing QA Brief

- **Date:** 2026-05-27T00:00:00Z
- **Author:** Verin
- **Scope:** Task 7 only. Create `Aiel.Permissions.Testing` (src) and `Aiel.Permissions.Testing.UnitTests` (tests). No product code changes. No changes to any existing Permissions project.
...
- **Baseline:** Tasks 5+6 landed and validated. 17/17 tests pass in `Aiel.Permissions.Application.UnitTests`. Worktree is dirty with uncommitted Tasks 5+6 work; Task 7 is stacked on top.
...
- `Aiel.Permissions.Testing` depends only on `Aiel.Permissions.Application.Contracts` and (optionally) `Aiel.Testing`. Both are present and stable.
- `Aiel.Permissions.Testing` does **not** require `Aiel.Permissions.Application` (the impl). The helpers implement and consume the **contracts** layer only; they provide fakes *of* those interfaces, not of the implementations.
- `Aiel.Permissions.Testing.UnitTests` tests only the helpers themselves. No behavioral gate or manager tests are duplicated here.
...
All fixture types shipped in `Aiel.Permissions.Testing` MUST obey these rules:
...
| Class names | `FakePermissionStore`, `FakeExecutionContextFactory`, `PermissionTestData` | `SamplePermissionStore`, `ExamplePermissions` |

Fixture action types MUST be declared in the `Aiel.Permissions.Testing` namespace (or a clearly test-only sub-namespace). They MUST implement only `IAction` and MUST NOT carry properties that model real domain concepts.
...
### `Aiel/src/Aiel.Permissions.Testing/Aiel.Permissions.Testing.csproj`

| Allowed references | Forbidden references |
|---|---|
| `Aiel.Permissions.Application.Contracts` | `Aiel.Permissions.Application` (the impl) |
...
### `Aiel/tests/Aiel.Permissions.Testing.UnitTests/Aiel.Permissions.Testing.UnitTests.csproj`

References `Aiel.Permissions.Testing`. Does NOT reference `Aiel.Permissions.Application` directly.

---

## Required helper surface

The following public types MUST exist in `Aiel.Permissions.Testing` in the `Aiel.Permissions` namespace:
...
1. `dotnet test .\Aiel\Aiel.slnx --nologo --tl:off -v minimal` passes cleanly. All 17 Tasks 5+6 tests still pass. New `Aiel.Permissions.Testing.UnitTests` tests also pass.
2. `Aiel.Permissions.Testing.csproj` does NOT reference `Aiel.Permissions.Application` or any infrastructure package.
3. Every public property on every public type in `Aiel.Permissions.Testing` is non-nullable and returns a valid non-default value (verified by dedicated surface tests described below).
...
## Required tests in `Aiel.Permissions.Testing.UnitTests`
...
- `Aiel.Permissions.Testing.csproj` carries a reference to `Aiel.Permissions.Application` (the impl). The package must be coupled to contracts only.
...
- Tests in `Aiel.Permissions.Testing.UnitTests` reference `Aiel.Permissions.Application` (the impl) directly. If interaction with the gate or manager is needed, use the inline fakes from that project's own test file or duplicate them locally; do not create a cross-project dependency.
