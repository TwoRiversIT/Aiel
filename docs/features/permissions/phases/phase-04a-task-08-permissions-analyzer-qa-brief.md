# Phase 04a Task 8 — Aiel.Permissions.Analyzers QA Brief

- **Date:** 2026-05-28T00:00:00Z
- **Author:** Verin
- **Scope:** Task 8 only. Add `Aiel.Permissions.Analyzers` (src) and `Aiel.Permissions.Analyzers.UnitTests` (tests). Add the `DoesNotRespectAuthority` attribute to `Aiel.Permissions.Application.Contracts`. No other product code changes.
- **Layer:** Roslyn analyzer package. Depends inward on `Microsoft.CodeAnalysis.CSharp` only. Identifies types by metadata name strings; no runtime project reference to `Aiel.Permissions.*` packages.
...
**Pre-req change required in Task 8 scope:** `DoesNotRespectAuthority` does not exist anywhere in the codebase. It MUST be defined in `Aiel.Permissions.Application.Contracts` as part of this slice. It is an application contract extension, not an analyzer implementation detail. The analyzer recognizes it by its fully qualified metadata name — no compile-time project reference between the analyzer DLL and the runtime package is required or permitted.
...
### `Aiel/src/Aiel.Permissions.Analyzers/Aiel.Permissions.Analyzers.csproj`
...
- Any `Aiel.*` project or package reference — the analyzer MUST NOT take a runtime dependency on `Aiel.Permissions.Application.Contracts` or any other Aiel package. All type recognition is by metadata name string.
...
### `Aiel/tests/Aiel.Permissions.Analyzers.UnitTests/Aiel.Permissions.Analyzers.UnitTests.csproj`
...
**Project reference:** `Aiel.Permissions.Analyzers` only.

---

## Diagnostic ID scheme

`Aiel.Permissions.Analyzers` occupies the `TRAF01000`–`TRAF01999` block.

| ID | Title | Severity | Suppressible |
|---|---|---|---|
| `TRAF01001` | `ActionHasNoAuthorizationStory` | `Error` | No |
| `TRAF01002` | `DoesNotRespectAuthorityReasonIsEmpty` | `Error` | No |

**`TRAF01001` — `ActionHasNoAuthorizationStory`**
...
Category: `Permissions`. DefaultSeverity: **Error**. Must NOT be suppressible via `#pragma warning disable` (enforce per D4: missing authorization story is always a build failure).

**`TRAF01002` — `DoesNotRespectAuthorityReasonIsEmpty`**

> `[DoesNotRespectAuthority]` on `{0}` has an empty or whitespace `Reason`. Provide a non-empty explanation.

Category: `Permissions`. DefaultSeverity: **Error**.
...
- For each named type that is a **concrete, non-abstract class** (not interface, not abstract, not struct) implementing `IActionInterfaceName` (`Aiel.Actions.IAction`):
  - Walks the compilation for any concrete non-abstract class implementing `IActionPermissionCheckerInterfaceName` (`Aiel.Permissions.IActionPermissionChecker<TAction>`) where `TAction` is the current action type. If found → no diagnostic.
  - Checks whether the action type carries `[DoesNotRespectAuthority]` (metadata name `Aiel.Permissions.DoesNotRespectAuthorityAttribute`). If present with a non-empty `Reason` → no diagnostic. If present with empty or whitespace `Reason` → `TRAF01002`.
  - If neither check passes → `TRAF01001`.
...
- Does NOT validate string-based permission name literals (future diagnostic `TRAF01003` or later).
...
1. `dotnet test .\Aiel\Aiel.slnx --nologo --tl:off -v minimal` passes cleanly. All previous tests still pass. New `Aiel.Permissions.Analyzers.UnitTests` tests pass.
2. `TRAF01001` has `DefaultSeverity = DiagnosticSeverity.Error` and is NOT in the suppression allow-list.
3. `TRAF01002` has `DefaultSeverity = DiagnosticSeverity.Error`.
4. `Aiel.Permissions.Analyzers.csproj` carries NO project or package reference to any `Aiel.*` package.
5. `DoesNotRespectAuthorityAttribute` exists in `Aiel.Permissions.Application.Contracts`, carries `required string Reason`, and has `AttributeUsage(AttributeTargets.Class, Inherited = false, AllowMultiple = false)`.
6. Both `TRAF01001` and `TRAF01002` appear in `AnalyzerReleases.Unshipped.md` with the correct metadata format.
...
- Add `DoesNotRespectAuthorityAttribute` to `Aiel.Permissions.Application.Contracts`.
...
## Required tests in `Aiel.Permissions.Analyzers.UnitTests`

All tests use inline source strings with `Aiel.Permissions.IAction`, `Aiel.Permissions.IActionPermissionChecker<TAction>`, and `Aiel.Permissions.DoesNotRespectAuthorityAttribute` stubs embedded as `TestState.Sources` entries — identical in structure to the `AssemblyAnalyzerTests.cs` pattern in `Aiel.Analyzers.UnitTests`. Tests DO NOT reference any `Aiel.*` runtime assembly.

### `ActionHasNoAuthorizationStoryTests`

1. `ReportsTRAF01001_WhenActionHasNoCheckerAndNoMarker`
   - Source: concrete class `SampleAction : IAction`. No checker. No attribute.
   - Expected: `TRAF01001` on the `SampleAction` type declaration.
...
6. `ReportsTRAF01001_ForEachUncoveredActionInCompilation`
   - Source: `SampleActionA : IAction`, `SampleActionB : IAction`, checker only for `SampleActionA`.
   - Expected: `TRAF01001` on `SampleActionB` only.

### `DoesNotRespectAuthorityReasonTests`

1. `ReportsTRAF01002_WhenReasonIsEmpty`
   - Source: `[DoesNotRespectAuthority(Reason = "")]` on `SampleAction : IAction`.
   - Expected: `TRAF01002` on `SampleAction`.

2. `ReportsTRAF01002_WhenReasonIsWhitespace`
   - Source: `[DoesNotRespectAuthority(Reason = "   ")]` on `SampleAction : IAction`.
   - Expected: `TRAF01002` on `SampleAction`.
...
2. `ReportsTRAF01001_WhenCheckerExistsForDifferentAction`
   ...
   - Expected: `TRAF01001` on `SampleActionB`.
...
Actions that have a generated permission definition (emitted by `Aiel.Permissions.Generators`) but no concrete checker and no `[DoesNotRespectAuthority]` marker will trigger `TRAF01001` in Task 8. This is an explicit, documented gap. It is NOT a Task 8 bug; it is the motivation for Task 9.

The `TRAF01001` diagnostic message MUST include a note that generated definitions are recognized after Task 9 lands:

> "Note: generated permission definitions are recognized by the analyzer only after `Aiel.Permissions.Generators` is added."

Task 9 will extend `ActionAuthorizationAnalyzer` (or a companion analyzer) to recognize the generated output and suppress `TRAF01001` accordingly.
...
- `Aiel.Permissions.Analyzers.csproj` contains a project or package reference to any `Aiel.*` assembly. Analyzers identify types by metadata name string only.
- `TRAF01001` or `TRAF01002` has `DefaultSeverity = DiagnosticSeverity.Warning`. Both MUST be errors per D4.
...
- `DoesNotRespectAuthorityAttribute` is not in `Aiel.Permissions.Application.Contracts`, or its `Reason` property is not `required`.
...
- `EnforceExtendedAnalyzerRules` is missing or set to `false` in `Aiel.Permissions.Analyzers.csproj`.
