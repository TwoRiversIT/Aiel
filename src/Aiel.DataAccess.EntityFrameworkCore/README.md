...
Provides opinionated, production-ready tooling for **migrating** and **seeding** databases with Entity Framework Core.
...
| Retry with jitter | `DatabaseMigratorBase` retries failed migrations up to *n* times (default 3) with a random back-off of 5Ã¢â‚¬â€œ15 s. |
...
You can install the Aiel.DataAccess.EntityFrameworkCore package via NuGet Package Manager Console:

```pwsh
Install-Package Aiel.DataAccess.EntityFrameworkCore
```

Or via .NET CLI:

```pwsh
dotnet add package Aiel.DataAccess.EntityFrameworkCore
```
...
```
IHost.MigrateAsync()
  Ã¢â€â€Ã¢â€â‚¬ IServiceProvider.MigrateAsync()
       Ã¢â€â€Ã¢â€â‚¬ IServiceScope.MigrateAsync()
            Ã¢â€â€Ã¢â€â‚¬ MigrationManager.MigrateAsync()
                 Ã¢â€â€Ã¢â€â‚¬ foreach IDatabaseMigrator in AielMigrationOptions.Migrators
                      Ã¢â€â€Ã¢â€â‚¬ IDatabaseMigrator.MigrateAsync()
                           Ã¢â€â€Ã¢â€â‚¬ DatabaseMigratorBase.TryAsync()  Ã¢â€ Â retry + tracing
                                Ã¢â€â€Ã¢â€â‚¬ DbContextMigrator<T>.ApplyMigrationsAsync()
```
...
- **`TryAsync`** Ã¢â‚¬â€ wraps a migration task with retry logic and an OpenTelemetry activity span.
  Retries up to `retryCount` times (default `3`) with a random delay of 5Ã¢â‚¬â€œ15 seconds between
...
// Registration Ã¢â‚¬â€ auto-discovered by AielEntityFrameworkCore.
